...
function hideTip() {
  ...
  // Hàm để gửi dữ liệu graph mới về backend
  async function handleGraphChange(newGraph) {
    console.log("Graph đã thay đổi, đang gửi về server...");
    try {
      ...
      console.log("Đã lưu vị trí minimap thành công!");
    } catch (err) {
      console.error("Lỗi khi lưu vị trí minimap:", err);
    }
  }
  ...
  const minimap = minimapEl
    ? createMinimap({
        container: minimapEl,
        graph: await fetch(`${dataBaseUrl}/graph`).then(r => r.ok ? r.json() : { nodes: [], edges: [] }),
        onGotoScene: (id) => { userActivity(); return navigateTo(id); },
        onPathPlay: async (path) => { /* ... */ },
        onGraphChange: handleGraphChange // <--- THÊM DÒNG NÀY
      })
    : null;
...
if (scenes && scenes.length > 0) {
  ...
  console.error("Không có scene nào để hiển thị.");
  // Optional: Hiển thị thông báo lỗi cho người dùng ở đây
}
  // ===== Return external API =====
  return {
    navigateTo,
